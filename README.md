...
```bash
docker build -t matlab:R2026b .
```

Run the container. Test the container by running an example MATLAB command, such as `ver`.
```bash
docker run --init --rm -e MLM_LICENSE_FILE=27000@MyServerName matlab:R2026b -batch ver
```
The [Dockerfile](https://github.com/mathworks-ref-arch/matlab-dockerfile/blob/main/Dockerfile) defaults to building a container for MATLAB R2026b.
...
|---|---|---|
| [MATLAB_RELEASE](#build-an-image-for-a-different-release-of-matlab) | R2026b | MATLAB release to install, for example, `R2023b`.|
...
| [MATLAB_INSTALL_LOCATION](#build-an-image-with-matlab-installed-to-a-specific-location) | /opt/matlab/R2026b | Path to install MATLAB. |
...
```bash
docker build --build-arg MATLAB_PRODUCT_LIST='MATLAB Simulink' -t matlab:R2026b .
```
...
```bash
docker build --build-arg MATLAB_INSTALL_LOCATION='/opt/matlab' -t matlab:R2026b .
```

#### Build an Image Based on a Specific OS

You can specify a base OS image to build upon using the `UBUNTU_VERSION` build argument. These should follow the convention used for the `mathworks/matlab-deps` images. For more information see the [list of matlab-deps base images](https://hub.docker.com/r/mathworks/matlab-deps#supported-tags). For example:

```bash
# Build a container based on a specific OS.
docker build --build-arg UBUNTU_VERSION=24.04 -t matlab:R2026b .
```
...
docker build --build-arg LICENSE_SERVER=27000@MyServerName -t matlab:R2026b .

# Run the container without needing to pass license information.
docker run --init --rm matlab:R2026b -batch ver
```
...
- Specify the `LICENSE_SERVER` build-arg.
    ...
    ```bash
    # Example
    docker build -t matlab:R2026b --build-arg LICENSE_SERVER=27000@MyServerName .
    ```
...
```bash
docker run --init -it --rm matlab:R2026b -logfile "logfilename.log"
```
...
For some workflows and toolboxes, you must specify dependencies. You must do this if you want to do these tasks:
    ...
    * Use the Polyspace&reg; 32-bit tcc compiler
    ...
    The [MATLAB Dependencies repository](https://github.com/mathworks-ref-arch/container-images/tree/main/matlab-deps) lists Dockerfiles for various releases and platforms. To view the Dockerfile for R2026b, click [here](https://github.com/mathworks-ref-arch/container-images/blob/main/matlab-deps/r2026b/ubuntu24.04/Dockerfile).

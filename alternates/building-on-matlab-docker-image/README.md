...
```bash
docker build -t matlab_with_add_ons:R2026b .
```
...
```bash
docker run --init --rm -e MLM_LICENSE_FILE=27000@MyServerName matlab_with_add_ons:R2026b -batch ver
```
...
```bash
docker run --init --rm -it -p 8888:8888 matlab_with_add_ons:R2026b -browser
```
...
```bash
docker build --build-arg ADDITIONAL_PRODUCTS="Deep_Learning_Toolbox" -t matlab_with_add_ons:R2026b .
```
...
```bash
docker build --build-arg FONTS_PACKAGES="fonts-arphic-gbsn00lp fonts-unfonts-core" -t matlab_with_add_ons:R2026b .
```
...
```bash
docker build --build-arg ADDITIONAL_APT_PACKAGES="vim" -t matlab_with_add_ons:R2026b .
```
...
```bash
docker build --build-arg LICENSE_SERVER=27000@MyServerName -t matlab_with_add_ons:R2026b .
```

Run the container, without needing to pass license information.
```bash
docker run --init matlab_with_add_ons:R2026b -batch ver
```
## Run the Container
...
```bash
docker run -it --rm -e LANG=ja_JP.UTF-8 --shm-size=512M matlab_with_add_ons:R2026b
```

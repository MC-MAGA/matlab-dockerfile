...
```bash
docker build -t matlab-non-interactive:R2026b .
```
...
```bash
docker run --rm matlab-non-interactive:R2026b matlab-batch -licenseToken "user@email.com::encodedToken" "rand"
```
...
```bash
docker build --build-arg MATLAB_PRODUCT_LIST="MATLAB Statistics_and_Machine_Learning_Toolbox" -t matlab-stats-non-interactive:R2026b .
```
...
```bash
docker build --build-arg MATLAB_INSTALL_LOCATION="/opt/matlab" -t matlab-non-interactive:R2026b .
```
...
- Specify the `-licenseToken` run-arg.
    ```bash
    # Example
    docker run --rm matlab-non-interactive:R2026b matlab-batch -licenseToken "user@email.com::encodedToken" "disp('Hello, World.')"
    ```
...
- Specify the `MLM_LICENSE_TOKEN` environment variable.
    ...
    docker run --rm -e MLM_LICENSE_TOKEN matlab-non-interactive:R2026b matlab-batch "disp('Hello, World.')"
    ```
...
export MLM_LICENSE_TOKEN="user@email.com::encodedToken"
docker run --rm -e MLM_LICENSE_TOKEN matlab-non-interactive:R2026b matlab-batch rand
```
...
export MLM_LICENSE_TOKEN="user@email.com::encodedToken"
docker run --rm -v $(pwd):/content -w /content -e MLM_LICENSE_TOKEN matlab-non-interactive:R2026b matlab-batch "myscript"
```

...
```bash
docker build -t mpm-archive:R2026b -f archive.Dockerfile .
```
...
```bash
DOCKER_BUILDKIT=1 docker build -t matlab-from-source:R2026b .
```
...
|---|---|---|
| [MATLAB_RELEASE](#build-an-archive-image-for-a-different-release-of-matlab) | R2026b | The MATLAB release to install, for example, `R2023b`. |
...
```bash
docker build --build-arg MATLAB_PRODUCT_LIST="MATLAB Statistics_and_Machine_Learning_Toolbox" -t mpm-archive:R2026b -f archive.Dockerfile .
```
...
| Argument Name | Default value | Effect |
|---|---|---|
| [MATLAB_RELEASE](#build-an-image-for-a-different-release-of-matlab) | R2026b | The MATLAB release you want to install, in lower-case. For example: `R2022a`. :warning: This release must match the `MATLAB_RELEASE` you use to build the archive image. |
...
| [MATLAB_INSTALL_LOCATION](#build-an-image-with-matlab-installed-to-a-specific-location) | /opt/matlab/R2026b | The path to install MATLAB. |
| [ARCHIVE_BASE_IMAGE](#build-an-image-from-a-different-archive) | mpm-archive:R2026b | The name of the Docker&reg; image containing the product installation files. |
...
```bash
docker build --build-arg MATLAB_PRODUCT_LIST="MATLAB Statistics_and_Machine_Learning_Toolbox" -t matlab-stats-from-source:R2026b .
```
...
```bash
docker build --build-arg MATLAB_INSTALL_LOCATION="/opt/matlab" -t matlab-from-source:R2026b .
```
...
```bash
docker build  --build-arg ARCHIVE_BASE_IMAGE=my-archive -t matlab-from-source:R2026b .
```
...
docker build --build-arg LICENSE_SERVER=27000@MyServerName -t matlab-from-source:R2026b .

# Run the container, without needing to pass license information.
docker run --init --rm matlab-from-source:R2026b -batch ver
```

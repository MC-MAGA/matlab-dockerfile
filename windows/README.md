...
```powershell
docker build -t matlab-on-windows:R2026b .
```
This [Dockerfile](./Dockerfile) defaults to building a Windows container for MATLAB R2026b. The Dockerfile is based on the Windows base image which contains the full Windows API set. For details, see the documentation on DockerHub for [Windows base image](https://hub.docker.com/r/microsoft/windows).

Test the container by running an example MATLAB command, such as `ver`. The entry point of the container is PowerShell.
```powershell
docker run --rm matlab-on-windows:R2026b matlab-batch "-licenseToken" "user@email.com::encodedToken" "ver"
```
...
```powershell
docker build --build-arg MATLAB_PRODUCT_LIST="MATLAB Statistics_and_Machine_Learning_Toolbox" -t matlab-stats-windows:R2026b .
```
...
docker run --rm matlab-on-windows:R2026b matlab-batch -licenseToken 'user@email.com::encodedToken' '\"disp(''hello world'')\"'
```
...
docker run --mount "type=bind,src=C:\scripts,target=C:\pwd" --workdir "C:\pwd" --rm matlab-on-windows:R2026b matlab-batch -licenseToken 'user@email.com::encodedToken' 'myscript'
```
...
```powershell
# Start MATLAB, display 'hello world', and exit.
docker run -e MLM_LICENSE_TOKEN='user@email.com::encodedToken' --rm matlab-on-windows:R2026b matlab-batch '\"disp(''hello world'')\"'
```
